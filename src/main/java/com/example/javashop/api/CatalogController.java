...
import com.example.javashop.api.dto.ProductCreateRequest;
...
public class CatalogController {
    ...
    public ResponseEntity<?> addProduct(@RequestBody ProductCreateRequest request) {
        try {
            ProductResponse resp = catalogService.addDynamicProduct(request);
    ...
    public ResponseEntity<?> deleteProduct(@PathVariable String id) {
        boolean removed = catalogService.deleteProduct(id);
        if (removed) {
